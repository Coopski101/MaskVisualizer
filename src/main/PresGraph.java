...
public class PresGraph extends MaskGraphs implements Serializable{

    public PresGraph(double m, double b, double rSq,  double[][] points){
        super( m,  b,  rSq,  points);
    }
    
    @Override
    public JFreeChart createChart(XYDataset dataSet){//implementation from abstract method
        JFreeChart chart = ChartFactory.createScatterPlot("Pressure VS Particles <= 5um", "Pressure: Pa", "# Particles", 
    ...
    protected JFreeChart reCreateChart(XYDataset dataSet, Double resultY, boolean extra){
        JFreeChart newChart = ChartFactory.createScatterPlot("Pressure VS Particles <= 5um", "Pressure: Pa", "# Particles",
            dataSet, PlotOrientation.VERTICAL, true, false, false);
